...
def fullstack_readme(project_name: str, project_description: str | None) -> str:
    return (
        ...
        f"│   ├── src/\n"
        f"│   │   ├── apps/           # One folder per feature app — pages live here\n"
        f"│   │   │   └── <app>/\n"
        f"│   │   │       ├── views/      # Page views, e.g. HomeView.vue\n"
        f"│   │   │       ├── router/     # index.ts — picked up by the root router\n"
        f"│   │   │       ├── stores/     # Pinia stores for this app\n"
        f"│   │   │       └── components/ # atoms/, molecules/, organisms/\n"
        f"│   │   ├── router/         # Root router; auto-globs apps/*/router/index.ts\n"
        f"│   │   ├── shared/         # Cross-app code (services/api.ts, stores, pages)\n"
        f"│   │   ├── components/     # Cross-app Vue components\n"
        f"│   │   ├── stores/         # Cross-app Pinia stores\n"
        f"│   │   └── services/       # Cross-app API services\n"
        f"│   └── package.json\n"
        f"├── backend/django/         # Django backend application\n"
        f"│   ├── apps/<app>/        # One Django app per feature app\n"
