...
from apps.Imagi.Build.ProjectManager.services.project_management_service import (
...
class ProjectCreateSerializerTests(APITestCase):
    ...
    def test_duplicate_active_name_rejected(self):
        ...
        from apps.Imagi.Build.ProjectManager.api.serializers import (
...
class ProjectManagerAPITests(APITestCase):
    ...
    @patch('apps.Imagi.Build.ProjectManager.api.views.start_initial_build')
    @patch(
        'apps.Imagi.Build.ProjectManager.api.views.ProjectCreationService.create_project'
    )
    ...
    @patch(
        'apps.Imagi.Build.ProjectManager.api.views.start_initial_build',
    ...
    @patch(
        'apps.Imagi.Build.ProjectManager.api.views.ProjectCreationService.initialize_project'
    )
...
class InitialBuildServiceTests(TestCase):
    ...
    def test_skips_without_api_key(self):
        from apps.Imagi.Build.ProjectManager.services import initial_build_service

        with patch(
            'apps.Imagi.Build.Agents.services.base_agent.OPENAI_API_KEY', None
        ):
    ...
    def test_marks_generating_and_spawns_thread(self):
        ...
        with patch(
            'apps.Imagi.Build.Agents.services.base_agent.OPENAI_API_KEY', 'test-key'
    ...
    def _run_with_agent_result(self, result):
        ...
        with patch(
            'apps.Imagi.Build.Agents.services.base_agent.ImagiAgentService',
